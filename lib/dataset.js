...
export function buildEntryFromDatasetItem(entry) {
  ...
  const avatar = entry?.creator?.avatar || null;
  // Optional fields the dev pages (label.js) substitute into the datamatrix,
  // description, and barcode; scenes/prerender ignore the extra keys.
  const description = entry?.metadata?.description || '';
  const instagram = entry?.creator?.instagram || null;
  const contractAddress = entry?.chaindata?.contractAddress || null;
  const tokenId = entry?.chaindata?.tokenId ?? null;
  const createdAtIso = entry?.chaindata?.createdAtIso || '';
  const sizeKb = Number(entry?.metadata?.sizeKb);
  if (
    !username
    || !name
    || !filename
  ...
  return {
    username, name, width, height, filename, avatar, description, instagram,
    contractAddress, tokenId, createdAtIso,
    sizeKb: Number.isFinite(sizeKb) ? sizeKb : null,
  };
}

/**
 * Formats an artwork's file weight (KB) into the label's "Net wt" stamp lines.
 * Mirrors the static defaults in label.html: KB / 1000 to one decimal with a
 * comma decimal separator ("3,4 Mb"), and the rounded KB ("(3430 kilobytes)").
 * Returns null when there's no usable size so callers leave the stamp untouched.
 */
export function formatNetWeight(sizeKb) {
  if (!Number.isFinite(sizeKb) || sizeKb <= 0) return null;
  const mb = (sizeKb / 1000).toFixed(1).replace('.', ',');
  return { value: `${mb} Mb`, unit: `(${Math.round(sizeKb)} kilobytes)` };
}
...
/**
 * Fuzzy-find a prerender-manifest entry by its author (username), matching the
 * baked-only carousel. Exact match first, then substring. Returns null on miss.
 */
export function findArtistInManifest(entries, query) {
  const q = String(query || '').toLowerCase().trim();
  if (!q) return null;
  return (
    entries.find((e) => String(e?.author || '').toLowerCase() === q) ||
