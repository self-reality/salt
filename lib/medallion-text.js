 ...
 * @param {boolean} [opts.showGuides] - When true, also renders the two baseline
 *   circles as a magenta dashed stroke so the text rings' geometry is visible.
 */
export function injectMedallionText(baseSvg, outerText, innerText, fontDataUrl, opts = {}) {
  ...
  // Guide circles are dev-only overlays: full magenta circles centred on the
  // disc at each baseline radius, drawn under the text so glyph readability is
  // preserved. Dashes make them legible against the disc fill at any radius.
  const guides = showGuides
    ? `<circle cx="${CX}" cy="${CY}" r="${outerBaselineR}" fill="none" ` +
        `stroke="#ff00ff" stroke-width="2" stroke-dasharray="6 4"/>` +
      `<circle cx="${CX}" cy="${CY}" r="${innerBaselineR}" fill="none" ` +
        `stroke="#ff00ff" stroke-width="2" stroke-dasharray="6 4"/>`
    : '';
  const injection =
    `<defs>` +
