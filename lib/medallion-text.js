...
// from the upper-left to the upper-right, so glyphs read upright over the top.
// The author arc goes clockwise through 6 o'clock from the lower-right to the
// lower-left — the classic medallion convention, where glyph tops face inward
// toward the disc centre.
//
...
// = clockwise on screen, so the path runs from (centre − span/2) clockwise
// through centre to (centre + span/2). large-arc flips when the span exceeds
// a semicircle so the arc still passes through the centre angle.
function arcPath(R, centreRad, spanRad) {
  const half = spanRad / 2;
  const a1 = centreRad - half;
  const a2 = centreRad + half;
...
export function injectMedallionText(baseSvg, outerText, innerText, fontDataUrl, opts = {}) {
  ...
  const defs =
    `<defs>` +
      (hasTitle ? `<path id="medallion-outer-arc" d="${arcPath(radius, TITLE_CENTRE_RAD, titleSpan)}"/>` : '') +
      (hasAuthor ? `<path id="medallion-inner-arc" d="${arcPath(radius, AUTHOR_CENTRE_RAD, authorSpan)}"/>` : '') +
    `</defs>`;
