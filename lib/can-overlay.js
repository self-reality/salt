...
export function addCanOverlay({ canGroup, url, u, v, wPx, hPx, texSize = 4096 }) {
  ...
  const material = new THREE.MeshStandardMaterial({
    ...
    polygonOffset: true,        // win the depth test against the coplanar surface,
    polygonOffsetFactor: -1,    // so the decal can sit flush instead of floating
  ...
  // Minimal lift along the normal — polygonOffset handles z-fighting, so this is
  // just enough to keep the decal off the exact surface plane. It hugs the can
  // instead of hovering (the previous, larger offset read as "not stuck").
  const offset = Math.max(width, height) * 0.004;

  function update() {
    // The texture coordinate is the source of truth: the decal center sits
    // wherever its UV lands on the current (deformed) mesh — barycentric over
    // the same triangle. Orientation stays frozen (the back face doesn't rotate
    // under the rigid stretch), so the decal never tilts off the surface.
    _p0.fromBufferAttribute(hit.posAttr, hit.ia);
    _p1.fromBufferAttribute(hit.posAttr, hit.ib);
    _p2.fromBufferAttribute(hit.posAttr, hit.ic);
    mesh.position.set(0, 0, 0)
      .addScaledVector(_p0, hit.w[0])
      .addScaledVector(_p1, hit.w[1])
      .addScaledVector(_p2, hit.w[2])
      .addScaledVector(restNormal, offset);
  }
  update();
