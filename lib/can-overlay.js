...
/**
 * Evaluates the anchor once, at rest: barycentric world position, surface
 * orientation, the local UV->world scale lengths (for sizing), and the outward
 * normal. Read from the current vertex positions (rest, at creation time).
 */
function computeAnchor(hit) {
  ...
  const normal = new THREE.Vector3().crossVectors(_e1, _e2).normalize();
  ...
  _x.copy(_dPdu).addScaledVector(normal, -_dPdu.dot(normal)).normalize();
  _y.crossVectors(normal, _x);
  _basis.makeBasis(_x, _y, normal);
  const quaternion = new THREE.Quaternion().setFromRotationMatrix(_basis);

  return { position, quaternion, normal, dPduLen: _dPdu.length(), dPdvLen: _dPdv.length() };
}

/**
 * Picks the triangle vertex whose UV is closest to (u,v). The stretch deforms
 * the can as two rigid halves split along a center plane; a single vertex always
 * lives wholly in one half, so tracking its rigid translation keeps the decal
 * glued without the tearing/tilting you'd get from re-fitting the whole triangle.
 * Choosing by UV (not 3D) distance keeps it on the anchor's side of the seam.
 */
function nearestVertexByUV(hit, u, v) {
  const idx = [hit.ia, hit.ib, hit.ic];
  let best = 0, bestD = Infinity;
  for (let i = 0; i < 3; i++) {
    const du = hit.uv[i][0] - u, dv = hit.uv[i][1] - v;
    const d = du * du + dv * dv;
    if (d < bestD) { bestD = d; best = i; }
  }
  const index = idx[best];
  const rest = new THREE.Vector3().fromBufferAttribute(hit.posAttr, index);
  return { posAttr: hit.posAttr, index, rest };
}
...
export function addCanOverlay({ canGroup, url, u, v, wPx, hPx, texSize = 4096 }) {
  ...
  // Rest anchor: position, frozen orientation, and the UV->world scale used to
  // size the plane. Orientation is computed once — rigid translation never
  // rotates the flat back face, so re-deriving it each frame only invites tilt.
  const anchor = computeAnchor(hit);
  const restAnchor = anchor.position.clone();
  const restNormal = anchor.normal.clone();
  const quaternion = anchor.quaternion;

  // Vertex used to follow the rigid translation of this patch of surface.
  const ref = nearestVertexByUV(hit, u, v);

  // Texture-pixel size -> world size via the local UV->world scale at rest.
  const width = (wPx / texSize) * anchor.dPduLen;
  const height = (hPx / texSize) * anchor.dPdvLen;
  ...
  // (Re)rasterise a source (url or data URL) into the decal texture. Used at
  // creation and whenever the decal is recoloured (e.g. the header palette).
  function setImage(src) {
    ...
    img.onerror = () => console.error('addCanOverlay: failed to load', src);
    img.src = src;
  }
  setImage(url);
  ...
  mesh.renderOrder = 1;
  mesh.quaternion.copy(quaternion);   // frozen orientation
  ...
  function update() {
    // Follow the rigid translation of the reference vertex (current - rest),
    // applied to the perfectly-placed rest anchor. Keeps the decal flush and
    // un-tilted across the center seam while staying constant size.
    _cur.fromBufferAttribute(ref.posAttr, ref.index).sub(ref.rest);
    mesh.position.copy(restAnchor).add(_cur).addScaledVector(restNormal, offset);
  }
  update();

  return { mesh, update, setImage };
}
