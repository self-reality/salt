...
import { createLabelTexture, bandHeightForArtwork, ELEMENT_FILES, loadInterFont } from './label-texture.js';
...
export function loadLabelAssets({ elementsBasePath = DEFAULT_ELEMENTS_BASE_PATH } = {}) {
  ...
  const p = (async () => {
    ...
    // Pacifico (SVG side/medallion text) + Inter (canvas curation caption) both
    // register with document.fonts; prerender awaits document.fonts.ready.
    const [pacificoDataUrl] = await Promise.all([loadPacificoDataUrl(), loadInterFont()]);
    return {
      elementSvgs,
      pacificoDataUrl,
...
export function createLabelBuild(assets, { resolution } = {}) {
  ...
  return {
    ...
    setPairGap(px) { builder.setPairGap(px); },
  };
}
