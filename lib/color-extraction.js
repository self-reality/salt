...
//
// Four methods are offered (see deriveColors). They differ mainly in how the
// background / accent is chosen; all of them run the same final pass so the
...
function methodVibrant(pixels, { paletteSize }) {
  const palette = medianCut(pixels.slice(), paletteSize);
  const total = palette.reduce((n, p) => n + p.count, 0) || 1;
  let accent = palette[0].rgb, best = -1;
  for (const { rgb, count } of palette) {
    const [, s, l] = rgbToHsl(rgb);
    // saturated, not too dark/light, with meaningful coverage
    const score = s * (1 - Math.abs(l - 0.5)) * Math.sqrt(count / total);
    if (score > best) { best = score; accent = rgb; }
  }
  const background = adjustSaturation(accent, 1.1); // vivid background
  const text = [255, 255, 255];                     // fixed by contrast pass
  return { background, text };
}

function methodExtremes(pixels, { paletteSize }) {
  const palette = medianCut(pixels.slice(), paletteSize).slice();
  palette.sort((a, b) => relativeLuminance(a.rgb) - relativeLuminance(b.rgb));
  const darkest = palette[0].rgb;
  const lightest = palette[palette.length - 1].rgb;
  const background = palette[palette.length >> 1].rgb; // mid-tone
  const text = relativeLuminance(background) < 0.5 ? lightest : darkest;
  return { background, text };
}
...
const METHODS = {
  ...
  vibrant: methodVibrant,
  extremes: methodExtremes,
};
 ...
 * @param {object} [opts]
 * @param {'dominant'|'average'|'vibrant'|'extremes'} [opts.method]
...
export function deriveColors(image, {
  ...
  let { background, text } = fn(pixels, { paletteSize: Math.max(2, Math.round(paletteSize)) });
