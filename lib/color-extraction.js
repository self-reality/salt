...
//
// The "dominant" median-cut method lives here (see deriveColors). It picks the
// largest quantised bucket as the background and the highest contrast×vividness
// bucket as the text. A shared final pass enforces the panel sliders
// (saturation, min-contrast): the text is pushed until it meets a WCAG contrast
// ratio against the background. The method tries to extract all three colours
// from the artwork, including the outline; when no suitable third colour is
// found, the outline falls back to a near-black/near-white edge legible against
// the text. The "vibrant-js" method (see lib/vibrant-method.js) is the other
// path the label panel offers, and uses the same finalize pass via finalizeTrio.
//
...
// Returns a raw { background, text, outline } trio extracted from the artwork
// (outline may be null when no suitable third colour is found); the outline +
// final passes are applied by deriveColors.
 ...
 * @param {number} [opts.vividness]   - text contrast↔saturation bias
...
export function deriveColors(image, {
  ...
  const { background, text, outline } = methodDominant(pixels, {
