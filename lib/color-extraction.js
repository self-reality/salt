...
//
// The "average" method picks the artwork's mean colour as the background and
// derives an opposing text + outline pair. The shared final pass enforces the
// panel sliders (saturation, min-contrast): text is pushed until it meets a
// WCAG contrast ratio against the background. When no suitable third colour
// is found, the outline falls back to a near-black/near-white edge legible
// against the text.
//
 ...
 * @param {number} [opts.hueShift]    - text hue rotation in degrees
...
export function deriveColors(image, {
  ...
  const { background, text, outline } = methodAverage(pixels, { hueShift });
