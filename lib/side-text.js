...
// fetch fails, text falls back to system cursive without breaking the page.
// -----------------------------------------------------------------------------
...
// Fetch the local Pacifico woff2 once and resolve to a data URL. Resolves to
// null on failure so generateSideTextSvg falls back to system cursive.
...
// Frame dimensions. Both labels share the same width + font size so they line
// up on the same vertical column when the side of the can is read (rotated
// 90° CCW). The original outlined SVGs were 38 wide for Preserved and 51 for
// the title — the LAYERS x coords in lib/label-texture.js were adjusted to
// re-centre Preserved on the title's column when this widened.
export const PRESERVED_FRAME = { width: 51, height: 156, fontSize: 38 };
