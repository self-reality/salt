...
function layoutLeftCol(u, colW, wrapTitle) {
  ...
  // Footer: 0.75u padding-y, then logo + footnote, vertically centred.
  // Full analogy with the serving/metrics rows — the "row content" is
  // logo + gap + widest unbreakable footnote word. When that natural width
  // exceeds innerW, the logo and footnote font scale uniformly so the row
  // fits; the gap stays fixed (same role as MIN_VALUE_GAP_U). Padding-y
  // stays unscaled too (same role as the section gaps elsewhere).
  blockY = y;
  const logoBaseSize = LOGO_SIZE_U * u;
  const footnoteBaseSize = FOOTNOTE_FONT_U * u;
  ...
  for (const line of FOOTNOTE_LINES) {
    for (const w of line.split(/\s+/)) {
      ...
      widestFootnoteWord = Math.max(
        widestFootnoteWord,
        measureWidth(w, footnoteBaseSize, 'normal'),
      );
    }
  }
  const footerNeeded = logoBaseSize + footerGap + widestFootnoteWord;
  const footerScale = footerNeeded > innerW
    ? Math.max(0, innerW - footerGap) / (logoBaseSize + widestFootnoteWord)
    : 1;
  const logoSize = logoBaseSize * footerScale;
  const footnoteSize = footnoteBaseSize * footerScale;
