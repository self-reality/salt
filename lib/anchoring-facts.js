...
export function createAnchoringFacts({ basePath = 'elements/' } = {}) {
  ...
  // For a given (u, L), pick the left column width so the right column's wrapped
  // content fits inside the left column's height — that keeps the natural canvas
  // height at its calibrated value (left.height + 2*frameBorder ≈ THICKNESS) at
  // every L, so the downstream slot-fit never has to squash anything vertically.
  //
  // Above the emergent threshold the left column uses its full 56.3u; below it,
  // leftColW shrinks 1:1 with L so every pixel the canvas loses goes to the
  // right column (which is where the height pressure comes from).
  function chooseLayout(u, L) {
    const b = FRAME_BORDER_U * u;
    const pad = COL_PADDING_U * u;
    const fullColW = COL_TITLE_WIDTH_U * u;

    function tryColW(colW) {
      const left = layoutLeftCol(u, colW);
      const innerCW = Math.max(1, L - 2 * b - colW - pad);
      const right = layoutRightCol(u, innerCW);
      return { leftColW: colW, left, innerCW, right };
    }

    const atFull = tryColW(fullColW);
    if (atFull.right.height <= atFull.left.height) return atFull;

    // Right column overflows at full leftColW. Binary search for the widest
    // leftColW where right.height ≤ left.height. Layout calls are pure
    // measurement (no rasterisation), so ~24 of them per render is cheap.
    let lo = 0;
    let hi = fullColW;
    let best = tryColW(0);
    // If even at colW=0 right can't fit, the table is too narrow to fully
    // accommodate; return what we have and accept some squash at this size.
    if (best.right.height > best.left.height) return best;

    for (let i = 0; i < 14; i++) {
      const mid = (lo + hi) / 2;
      const c = tryColW(mid);
      if (c.right.height <= c.left.height) { lo = mid; best = c; }
      else { hi = mid; }
    }
    return best;
  }
  ...
  function totalHeight(L, u) {
    const { left, right } = chooseLayout(u, L);
  ...
  function render(length /* , colors */) {
    ...
    const u = calibratedU;
    const { leftColW, left, right } = chooseLayout(u, L);
  ...
  function getLayout(length) {
    ...
    const b = FRAME_BORDER_U * u;
    const L = Math.max(1, Math.round(length));
    const { leftColW } = chooseLayout(u, L);
    return {
      u,
      frameBorder: b,
      leftColStart: b,
  ...
  function getInspect(length) {
    ...
    const pad = COL_PADDING_U * u;
    const L = Math.max(1, Math.round(length));
    const { leftColW: colW, left, innerCW: innerContentW, right } = chooseLayout(u, L);
    const contentH = Math.max(left.height, right.height);
