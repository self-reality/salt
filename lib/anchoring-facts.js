...
// Inset (texture px) from the band's top and bottom edges, so the table doesn't
// run flush to the decal edge. Applied along the length (reflow) axis: the table
// reflows to band height − 2·PAD and is placed PAD below the top edge.
...
// The table's authored ink sentinel. Unlike the rest of the decal (which recolours
// #272727 -> the palette text), the Anchoring-facts panel is a fixed black-on-white
// nutrition sticker: render() forces this ink to pure black, never the palette.
const INK = /#272727/gi;
...
export function createAnchoringFacts({ basePath = 'elements/' } = {}) {
  ...
  // It's a Shadow DOM: the table's CSS carries the page's own chrome (a `body {
  // display:flex; flex-direction:column; … }` rule, etc.), and a plain <style> in
  // the document would leak that globally and hijack the host page's layout — so it
  // must be encapsulated. The host element still has to be connected for layout.
  ...
  async function load() {
    ...
    // Inline the footer logo as a data URL so the rasterised SVG has no external
    // sub-resource to fetch. It's a flat black raster, which suits this fixed
    // black-on-white panel (recolouring it via CSS mask broke the SVG-as-image
    // rasterisation, so it's drawn as a plain <img>).
  ...
  function render(length, colors) {
    ...
    // Always black-on-white: the panel reads as a printed nutrition sticker and
    // deliberately ignores the artwork palette (`colors`). The frame's #fff
    // background is left untouched (INK only matches #272727), so paper stays white.
    const ink = '#000000';
      ...
      `<svg xmlns="http://www.w3.org/2000/svg" width="${L}" height="${spine}">` +
        `<foreignObject x="0" y="0" width="${L}" height="${spine}">` +
          `<div xmlns="http://www.w3.org/1999/xhtml" style="margin:0">` +
            // The stylesheet is embedded inside the SVG's XML, so wrap it in CDATA:
          // otherwise any `<` in the CSS (e.g. a tag name mentioned in a comment) is
          // parsed as a tag and breaks the whole SVG. The /* */ keep it valid CSS too.
          `<style>/*<![CDATA[*/${css.replace(INK, ink)}/*]]>*/</style>` +
