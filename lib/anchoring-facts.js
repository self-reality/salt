...
// Below this canvas width, the left column gives up width 1:1 with the canvas's
// width loss — every pixel the canvas loses goes entirely to the right column,
// which keeps the right column's innerCW (and therefore its wrap count and
// height) constant. That keeps the natural canvas height at the value it had
// at the breakpoint, so the downstream slot-fit step never has to squash.
//
// Set just above the L where the right column's wrapped content first starts
// to exceed the left column's height at full leftColW; below the breakpoint,
// the right column never has less width than it had at the breakpoint.
...
// Below this canvas width, the footer's logo and footnote (and the gap and
// padding around them) scale uniformly by L / FOOTER_SCALE_BELOW_PX so the
// footer doesn't visually dominate the increasingly-narrow left column.
const FOOTER_SCALE_BELOW_PX = 728;
...
// shrunk linearly below). Contains four sections separated by 1.5u gaps:
...
// `footerScale` (default 1) shrinks the footer's logo, footnote, gap and
// padding-y uniformly for narrow canvases — see FOOTER_SCALE_BELOW_PX.
function layoutLeftCol(u, colW, wrapTitle, footerScale = 1) {
  ...
  // centred. Below FOOTER_SCALE_BELOW_PX the caller passes footerScale < 1 and
  // every footer dimension (logo, footnote font, gap, padding-y) shrinks by
  // the same factor — the two elements scale as one block.
  blockY = y;
  const footerPadY = FOOTER_PADDING_Y_U * u * footerScale;
  const logoSize = LOGO_SIZE_U * u * footerScale;
  const footerGap = FOOTER_GAP_U * u * footerScale;
  const footnoteSize = FOOTNOTE_FONT_U * u * footerScale;
...
export function createAnchoringFacts({ basePath = 'elements/' } = {}) {
  ...
  // Linear left-column width: full 56.3u above the breakpoint; below it, each
  // pixel of canvas-width loss subtracts a pixel from leftColW (1:1), so the
  // right column's innerCW stays at whatever value it had at the breakpoint.
  // Constant innerCW for the right column ⇒ constant wrap count ⇒ constant
  // right.height ⇒ constant natural canvas height.
  function leftColWFor(L, u) {
    const fullColW = COL_TITLE_WIDTH_U * u;
    if (L >= RESPONSIVE_BREAKPOINT_PX) return fullColW;
    return Math.max(0, L - RESPONSIVE_BREAKPOINT_PX + fullColW);
  }
  ...
  function totalHeight(L, u) {
    ...
    const footerScale = L < FOOTER_SCALE_BELOW_PX ? L / FOOTER_SCALE_BELOW_PX : 1;
    const left = layoutLeftCol(u, leftColW, wrapTitle, footerScale);
  ...
  function getInspect(length) {
    ...
    const wrapTitle = L <= TITLE_WRAP_AT_OR_BELOW_PX;
    const footerScale = L < FOOTER_SCALE_BELOW_PX ? L / FOOTER_SCALE_BELOW_PX : 1;
    const left = layoutLeftCol(u, colW, wrapTitle, footerScale);
