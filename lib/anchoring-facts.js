...
// Below this canvas width, both columns scale by the same factor (L /
// breakpoint), so they shrink together and the left column never collapses to 0
// ahead of the right one — see leftColWFor. Because the right column narrows
// too, its text wraps onto more lines and the table's natural height grows; the
// downstream slot-fit step (label-texture.js) then squashes the table across the
// can to fit its fixed slot. That squash is accepted in exchange for the
// balanced column behaviour.
const RESPONSIVE_BREAKPOINT_PX = 870;

// At or below this canvas width, the title wraps to one word per line
// ("Anchoring" / "facts"). Above, it stays on a single line (auto-shrunk to
// fit). Slightly below RESPONSIVE_BREAKPOINT_PX so the column-shrink kicks in
// a hair before the title style switches — the small auto-shrink between the
// two thresholds is much less jarring than a sudden 1→2 line jump at the same
// width as the column starts moving.
const TITLE_WRAP_AT_OR_BELOW_PX = 860;
...
// Format the artwork's native pixel dimensions for the serving row, matching the
// placeholder style: per-dimension kilopixels, 2 decimals, comma decimal point,
// lowercase 'x', single trailing unit (9000×11373 → "9,00x11,37 Kpx"). Returns
// null for missing/invalid dims so callers can fall back to SERVING_ROW.value.
export function formatDimensions(width, height) {
  if (!Number.isFinite(width) || !Number.isFinite(height) || width <= 0 || height <= 0) return null;
  const k = (n) => (n / 1000).toFixed(2).replace('.', ',');
  return `${k(width)}x${k(height)} Kpx`;
}
...
// shrunk proportionally below). Contains four sections separated by 1.5u gaps:
// title block, serving block, metrics block, footer. Coordinates are relative
// to the column's outer top-left (i.e. INSIDE the frame border). `wrapTitle`
// switches the title between single-line auto-shrink (the original nutrition-
// label look, for wider canvases) and one-word-per-line wrap (for narrow ones).
function layoutLeftCol(u, colW, wrapTitle, servingValue = SERVING_ROW.value) {
  ...
  if (!wrapTitle || titleNaturalW <= innerW) {
    // Single line — at full size if it fits, otherwise shrunk to fit innerW.
    const titleSize = titleNaturalW > innerW
      ? titleDesired * (innerW / titleNaturalW)
      : titleDesired;
    ops.push({ k: 'text', x: innerX, y, size: titleSize, weight: 'bold',
               text: TITLE_TEXT, baseline: 'top' });
    y += titleSize;
  } else {
    // Wrap each word onto its own line ("Anchoring" / "facts"). Only if a
    // single word still overflows do we fall back to font-shrink — a safety
    // net for absurdly narrow widths.
  ...
  const sValueBase = measureWidth(servingValue, sBase, 'bold');
  ...
  const sSize = sNeeded > innerW
    ...
    : sBase;
  const sValueW = measureWidth(servingValue, sSize, 'bold');
  ...
  ops.push({ k: 'text', x: innerX + innerW - sValueW, y,
             ...
             text: servingValue, baseline: 'top' });
  y += sSize;
  y += SERVING_GAP_U * u;
  ...
  // Footer: 0.75u padding-y, then logo (rotated 90°) + footnote, vertically
  // centred. Full analogy with the serving/metrics rows — the "row content" is
  // logo + gap + widest unbreakable footnote word ("A.C.C.E.P.T.A.N.C.E.").
  // When that natural width exceeds innerW, the logo and footnote font scale
  // uniformly so the row fits; the gap stays fixed (same role as
  // MIN_VALUE_GAP_U), padding-y stays fixed (same role as the section gaps).
  blockY = y;
  const logoBase = LOGO_SIZE_U * u;
  const footnoteBase = FOOTNOTE_FONT_U * u;
  const footerGap = FOOTER_GAP_U * u;
  let widestFootnoteWord = 0;
  for (const line of FOOTNOTE_LINES) {
    for (const w of line.split(/\s+/)) {
      if (!w) continue;
      widestFootnoteWord = Math.max(
        widestFootnoteWord,
        measureWidth(w, footnoteBase, 'normal'),
      );
    }
  }
  const footerNeeded = logoBase + footerGap + widestFootnoteWord;
  const footerScale = footerNeeded > innerW
    ? Math.max(0, innerW - footerGap) / (logoBase + widestFootnoteWord)
    : 1;
  const logoSize = logoBase * footerScale;
  const footnoteSize = footnoteBase * footerScale;
  ...
  const footnoteMaxW = Math.max(1, innerW - logoSize - footerGap);
  const footnoteLines = [];
  for (const line of FOOTNOTE_LINES) {
  ...
  const footnoteX = innerX + logoSize + footerGap;
...
export function createAnchoringFacts({ basePath = 'elements/' } = {}) {
  ...
  let calibratedU = 8;
  let ready = false;
  // Serving-row value — defaults to the placeholder until setDimensions() feeds
  // in the current artwork's real native size.
  let dimsText = SERVING_ROW.value;

  // Left-column width: full 56.3u above the breakpoint; below it, both columns
  // scale by the same factor L / breakpoint, so leftColW / L stays constant and
  // the left column never collapses to 0 before the right one starts to narrow.
  function leftColWFor(L, u) {
    const fullColW = COL_TITLE_WIDTH_U * u;
    if (L >= RESPONSIVE_BREAKPOINT_PX) return fullColW;
    return fullColW * (L / RESPONSIVE_BREAKPOINT_PX);
  }
  ...
  function totalHeight(L, u) {
    const leftColW = leftColWFor(L, u);
    const wrapTitle = L <= TITLE_WRAP_AT_OR_BELOW_PX;
    const left = layoutLeftCol(u, leftColW, wrapTitle);
  ...
  function render(length /* , colors */) {
    ...
    const leftColW = leftColWFor(L, u);
    const wrapTitle = L <= TITLE_WRAP_AT_OR_BELOW_PX;
    const left = layoutLeftCol(u, leftColW, wrapTitle, dimsText);
    const innerContentW = Math.max(
      1, L - 2 * FRAME_BORDER_U * u - leftColW - COL_PADDING_U * u,
    );
    const right = layoutRightCol(u, innerContentW);
  ...
  function getInspect(length) {
    ...
    const L = Math.max(1, Math.round(length));
    const colW = leftColWFor(L, u);
    const wrapTitle = L <= TITLE_WRAP_AT_OR_BELOW_PX;
    const left = layoutLeftCol(u, colW, wrapTitle);
  ...
  // Set the serving-row dimensions from the artwork's native pixel size. Falls
  // back to the placeholder when dims are missing/invalid. Caller re-renders.
  function setDimensions(width, height) {
    dimsText = formatDimensions(width, height) || SERVING_ROW.value;
  }

  return { load, render, getLayout, getInspect, setDimensions };
}
