...
export function createAnchoringFacts({ basePath = 'elements/' } = {}) {
  ...
  async function load() {
    ...
    // from the inlined PNG, its colour from background-color, which the #272727 ->
    // text swap recolours to the live ink. So it follows the palette, unlike a flat
    // <img>. The data URL lives on the element (not the <style>), so the INK replace
    // in render() never touches it.
