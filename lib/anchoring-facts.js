...
export function createAnchoringFacts({ basePath = 'elements/' } = {}) {
  ...
  async function load() {
    ...
    // The logo is a CSS-masked solid fill (see .logo in the HTML): its alpha comes
    // from the inlined PNG, its colour from background-color (the #272727 ink, which
    // render() forces to black like the rest of the panel). The data URL lives on the
    // element (not the <style>), so the INK replace in render() never touches it.
    const logoEl = frameEl.querySelector('.logo');
    if (logoEl && logoUrl) {
      logoEl.style.setProperty('-webkit-mask-image', `url("${logoUrl}")`);
      logoEl.style.setProperty('mask-image', `url("${logoUrl}")`);
    }
