...
function layoutLeftCol(u) {
  ...
  // Auto-shrink the title if its natural width would spill past the column's
  // inner padding — preserves the single-line nutrition-label look.
  let blockY = y;
...
export function createAnchoringFacts({ basePath = 'elements/' } = {}) {
  ...
  function getLayout(length) {
    ...
    return {
      ...
      frameBorder: b,
      leftColStart: b,
      colDivider: b + COL_TITLE_WIDTH_U * u,
      rightColEnd: L - b,
      width: L,
    };
  }
  ...
  function getInspect(length) {
    ...
    const colW = COL_TITLE_WIDTH_U * u;
    const L = Math.max(1, Math.round(length));
