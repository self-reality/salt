 ...
 * @param {boolean} [opts.baked] - Opt into pregenerated-can mode. The decal
 *   canvas / base-image pipeline is skipped entirely; instead the material's map
 *   is set directly from a prebaked full-can texture via setBakedTexture(url).
 *   onLoaded then exposes preloadBakedTexture/setBakedTexture and fires as soon
 *   as the FBX is ready (no base image to wait for).
 * @param {function} opts.onLoaded  - Called with { canGroup, material, setArtwork, width, height, depth }
 */
export function loadCan({ modelPath, texturePath, labelBuild, baked, onLoaded }) {
  ...
  // Baked-can mode: prebaked full-can textures keyed by url. Each is configured
  // exactly like the legacy decal CanvasTexture (sRGB, flipY, anisotropy) so the
  // on-can result is identical to the live-composited path.
  const bakedCache = {};
  function makeBakedTexture(url) {
    if (bakedCache[url]) return bakedCache[url];
  ...
  /** Warms the cache (and kicks off the GPU upload) for a baked texture url. */
  function preloadBakedTexture(url) {
    makeBakedTexture(url);
  }
  /** Swaps the can's base-color map to the prebaked full-can texture at url. */
  function setBakedTexture(url) {
    material.map = makeBakedTexture(url);
    material.needsUpdate = true;
  }

  function tryOnLoaded() {
    if (!fbxReady) return;
    if (!baked && !baseImageReady) return;
  ...
  // Load base image into offscreen canvas. Skipped in baked mode: the prebaked
  // full-can texture already carries the base color, so there's no decal canvas
  // to seed and no point fetching salt-bitmap.png.
  const baseImage = baked ? null : new Image();
  if (baseImage) baseImage.src = texturePath + 'salt-bitmap.png';
  if (baseImage) baseImage.onload = () => {
