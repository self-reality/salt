...
export function createLabelTexture({ resolution = TEX_WIDTH } = {}) {
  ...
  function draw() {
    ...
    //    placed rotated 90° into its slot: its length fills the band height while
    //    its thickness is fit to the fixed slot width. Drawn first (back-most), as
    //    it was the back layer in the old manifest.
    if (anchoringImg) {
      const X = ANCHORING_X * s;
      const Lh = H * s;                     // band length, running down the texture
  ...
  function renderAnchoring() {
    ...
    anchoring.render(bandHeight, colors).then(
