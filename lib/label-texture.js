...
// The paths are fill="black" at opacity 0.06 and stay that way regardless of
// palette — the 6% black just darkens whichever background colour is beneath.
export const BACKGROUND_FILE = 'Background.svg';
const BACKGROUND_REGION = { x: 1770, y: 1416, width: 4096, height: 2196 };

export function prepareBackgroundSvg(svgText) {
...
// anchored). The band must hold the three stacked with a 10 px gap above
// the Inner Circle and ≥ −5 px below it (a 5 px overlap with the pill is
// fine):
//
//     H = 322·k + 10 + 476·k + (−5) + 104·k = 902·k + 5
//
// so the natural-size fit threshold is TOP_TRIO_MIN_H = 907 (k=1). Below
// that we scale the trio uniformly by k = (H − 5) / 902; never above 1.
...
const TOP_TRIO_MIN_H = 907;
const INNER_CIRCLE_TOP_IN_SVG = 736.137;
// H at which the Circle's current centre anchor would push the Inner Circle
// bottom past Acceptance top − (−5), i.e. spacing₂ would dip below −5. From
// H/2 − 502.857 = −5. Above this we use the centre anchor unchanged; between
// this and TOP_TRIO_MIN_H we pin the Inner Circle bottom 5 px below the pill
// top so the overlap holds at exactly the −5 floor.
const TOP_TRIO_CENTER_ANCHOR_MIN_H = 995.71;
...
export function createLabelTexture({ resolution = TEX_WIDTH } = {}) {
  ...
  function draw() {
    ...
    const topScale = H < TOP_TRIO_MIN_H ? (H - 5) / 902 : 1;
    const innerTop =
      topScale < 1                          ? 322 * topScale + 10
      : H >= TOP_TRIO_CENTER_ANCHOR_MIN_H   ? H / 2 - 77.143
      :                                       H - 575;
  ...
  // Re-raster the decal-background pattern. One-shot per setElements/setElement
  // call — palette changes don't touch this layer (paths stay fill="black").
  function rebuildBackground(svgText) {
    if (!svgText) { backgroundImg = null; draw(); return; }
    rasterizeSvg(prepareBackgroundSvg(svgText), {
  ...
  return {
    ...
    setElements(svgByFile) {
      ...
      rebuildAll();
      if (svgByFile[BACKGROUND_FILE] != null) rebuildBackground(svgByFile[BACKGROUND_FILE]);
    },
    ...
    setElement(file, svgText) {
      if (file === BACKGROUND_FILE) { rebuildBackground(svgText); return; }
