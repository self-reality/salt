...
// bottom-anchored) all share centre x=3882 (= 3724 + 316/2 = 3726 + 312/2 =
// 3826.8 + 110.4/2). Their natural anchor offsets close the stamp→datamatrix
// gap at H≈922 and the barcode→stamp gap at H≈898; at H=930 the smaller of
// the two gaps is ~4 px — the floor below which the elements would overlap.
// So below H=RIGHT_COL_MIN_H we treat anchoring as if H were 930 and uniformly
// scale the trio around RIGHT_COL_CENTER_X by k = H / 930, preserving their
// relative spacing instead of letting them collide.
...
export function createLabelTexture({ resolution = TEX_WIDTH } = {}) {
  ...
  function draw() {
    ...
    //    The right-column trio is the exception: below RIGHT_COL_MIN_H the whole
    //    stack scales uniformly around its shared centre so the gaps don't close.
    ...
    for (const L of layers) {
      ...
      const x = rightCol ? RIGHT_COL_CENTER_X - drawW / 2 : L.x;
      const y = layerY(L, anchorH) * k;
      ctx.drawImage(
