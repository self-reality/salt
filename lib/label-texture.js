...
// A curation-status caption shares the left white column with the handle, set to
// its left and reading bottom-to-top (CCW) so it runs parallel to the handle. The
// caption + handle are centred in the column as a balanced pair (see draw()). The
// column is always white, so the caption is fixed black ink — not the per-artwork
// colours.text, which is tuned for the coloured background. It's drawn with pure
// Canvas 2D fillText (no SVG foreignObject) to keep the canvas WebGL-uploadable.
const IG_STATUS_TEXT = 'Curation passed — anchoring required';
// Inter (bundled woff2, registered with document.fonts by loadInterFont) with a
// system-sans fallback if it hasn't loaded yet.
const IG_STATUS_FONT = 'Inter, Helvetica, Arial, sans-serif';
const IG_STATUS_FONT_FILE = 'elements/fonts/Inter-Regular.woff2';
const IG_STATUS_FONT_MAX = 120;  // px in texture units, safety cap on the font size
const IG_STATUS_THICKNESS_FACTOR = 1; // caption thickness across the column ≈ fontPx
const IG_STATUS_LENGTH_VS_HANDLE = 2; // caption length = 2× the handle's max length
const DEFAULT_IG_PAIR_GAP = 36;  // texture-px gap between the caption and the handle

let interFontPromise = null;
// Fetch + register Inter once so the canvas-drawn curation caption renders in
// Inter both on the dev page and in headless prerender (which has no system
// fonts). Mirrors loadPacificoDataUrl() in side-text.js. Resolves regardless of
// success — on failure the caption falls back to the system sans stack.
export function loadInterFont() {
  if (interFontPromise) return interFontPromise;
  interFontPromise = (async () => {
    try {
      const res = await fetch(IG_STATUS_FONT_FILE, { cache: 'no-cache' });
      if (!res.ok) return;
    ...
    } catch (_) { /* canvas falls back to the system sans stack */ }
  })();
...
export function createLabelTexture({ resolution = TEX_WIDTH } = {}) {
  ...
  // Texture-px gap between the curation caption and the IG handle in the left
  // column — driven live from the side panel via setPairGap().
  let igPairGap = DEFAULT_IG_PAIR_GAP;
  ...
  function draw() {
    ...
    // Curation-status caption + Instagram-handle mark, sharing the left white
    // margin. The caption sits to the LEFT of the handle and reads bottom-to-top
    // (rotated -90°, like the Smiths / Anchoring blocks below), so it runs parallel
    // to the handle; the two are centred in the 296px column as a balanced pair.
    if (igHandleImg) {
      // Handle: fill the band height less the margin, but never upscale past the
      // PNG's native height (360 px).
      ...
      // Caption: its length axis is 2× the handle's max length (naturalHeight),
      // clamped to the band, and centred on the same midline as the handle — so it
      // reads as a deliberately larger sibling of the handle, not its full-height
      // rival. Font size fills that length (safety-capped); thickness ≈ fontPx.
      const len = Math.max(1, Math.min(
        H - 2 * IG_HANDLE_MARGIN,
        IG_STATUS_LENGTH_VS_HANDLE * igHandleImg.naturalHeight,
      ));
      ctx.save();
      ctx.font = `100px ${IG_STATUS_FONT}`;            // probe to get advance-per-em
      const adv = ctx.measureText(IG_STATUS_TEXT).width / 100;
      ctx.restore();
      const fontPx = adv > 0 ? Math.min(IG_STATUS_FONT_MAX, len / adv) : IG_STATUS_FONT_MAX;
      const Tw = fontPx * IG_STATUS_THICKNESS_FACTOR;

      // Centre [caption | gap | handle] as a group across the 296px column.
      const groupW = Tw + igPairGap + drawW;
      const leftStart = (IG_HANDLE_COLUMN_WIDTH - groupW) / 2;
      const xCaption = leftStart;                      // caption thickness-slot left edge
      const xHandle = leftStart + Tw + igPairGap;      // handle, to the caption's right
      const y = (H - drawH) / 2;                        // handle centred down the band

      // Caption — rotate -90° (CCW) and paint bottom-to-top, matching the handle.
      ctx.save();
      ctx.translate(xCaption * s, ((H + len) / 2) * s); // slot bottom, centred on the midline
      ctx.rotate(-Math.PI / 2);                               // local +x → up the band
      ctx.font = `${fontPx * s}px ${IG_STATUS_FONT}`;
      ctx.fillStyle = '#000000';
      ctx.textAlign = 'center';
      ctx.textBaseline = 'middle';
      ctx.fillText(IG_STATUS_TEXT, (len / 2) * s, (Tw / 2) * s);
      ctx.restore();

      ctx.drawImage(
        igHandleImg, 0, 0, igHandleImg.naturalWidth, igHandleImg.naturalHeight,
        xHandle * s, y * s, drawW * s, drawH * s,
      );
    }
    ctx.fillStyle = '#000000';
  ...
  return {
    ...
    // Push the artwork's native pixel size into the live anchoring-facts table
    // ("Original size" row). Re-renders that layer with the new value.
    setDimensions(width, height) { anchoring.setDimensions(width, height); renderAnchoring(); },
    ...
    // Texture-px gap between the curation caption and the IG handle (left column).
    setPairGap(px) {
      if (Number.isFinite(px)) igPairGap = Math.max(0, px);
      draw();
    },
