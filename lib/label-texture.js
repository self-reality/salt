...
// from its individual element SVGs (header, Smiths blurb, Anchoring-facts table,
// the SALT logo medallion, footer pill, datamatrix, …), recoloured to the live
// palette, with the artwork drawn into the Art slot. This is the canonical place
...
const LAYERS = [
  { name: 'anchoring-facts', file: 'Anchoring facts.svg',  x: 1856,    yTop: 0,      anchor: 'stretch' },
  ...
  // Circle.svg's viewBox was widened to y -23..1382 so its full text ring shows
  // (it was clipped to the 1033 frame); yTop accounts for that -23 viewBox top.
  { name: 'medallion',       file: 'Circle.svg',           x: 1996.71, yTop: -24,    anchor: 'center' },
...
export function createLabelTexture({ resolution = TEX_WIDTH } = {}) {
  ...
  function draw() {
    ...
    // 3) Element layers — each placed by its anchor and drawn at natural size, so
    //    it never distorts. 'stretch' layers fill the band height like the artwork.
  ...
  function rebuildLayer(L) {
    ...
    rasterizeSvg(recolorDecalSvg(L.svgText, colors), {
